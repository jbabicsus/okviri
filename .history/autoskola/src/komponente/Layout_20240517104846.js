...
function Layout({ children }) {
    ...
    const userMenu = [
        ...
        { name: 'Prijava instruktora', path: '/prijava-instruktor' },
    ...
    const instruktorMenu = [
        { name: 'Home', path: '/' },
        { name: 'Termini', path: '/termini' },
        { name: 'Profile', path: `/instruktor/profile/${user._id}` },
    ];

    const adminMenu = [
        { name: 'Home', path: '/' },
        { name: 'Users', path: '/admin/userslist' },
        { name: 'Instruktori', path: '/admin/instruktorlist' },
        { name: 'Profile', path: '/profile' },
        { 
            name: 'Logout', 
            path: '#', // Izmenjeno da ne navigira
                    ...
                    <div className='menu'>
                        {renderMenu.map((menu) => (
                            <div key={menu.name} className={`flex ${location.pathname === menu.path ? 'active-menu' : ''}`}>
                                <Link to={menu.path} className='linkovi' onClick={menu.onClick ? menu.onClick : null}>
                                    {menu.name}
                                </Link>
                            </div>
                        ))}
