...
function InstruktorForm({onFinish, initialValues}) {
    // Provjeri postoji li initialValues.timings i ima li barem dva elementa
    const initialTimings = initialValues && initialValues.timings && initialValues.timings.length === 2 
        ? [moment(initialValues.timings[0], 'HH:mm'), moment(initialValues.timings[1], 'HH:mm')] 
        : [];

    return (
        <Form layout='vertical' onFinish={onFinish} initialValues={{...initialValues, timings: initialTimings}}>
            <Row gutter={20}>
