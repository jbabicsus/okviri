...
function Instruktorlist() {
    ...
    const changeInstruktorStatus=async(record, status)=>{
        try{
            const response = await axios.get('/api/admin/get-all-instruktor', {instruktorId: record._id, userId: record._userI, status: status},{
                headers: {
                    Authorization : `Bearer ${localStorage.getItem('token')}`
                }
            })
