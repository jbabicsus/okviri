...
function Profile() {
    ...
    const onFinish = async (values) => {
        try {
            ...
            const response = await axios.post('/api/instruktor/apply-instruktor-account', { ...values, userId: user._id, timings: [moment(values.timigs[0]).format("HH:mm"), moment(values.timigs[1]).format("HH:mm")] }, {
                headers: {
